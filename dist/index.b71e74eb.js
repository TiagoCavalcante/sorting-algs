...
parcelHelpers.export(exports, "drawBarConfig", ()=>drawBarConfig);
const canvas = document.getElementById("canvas");
const context = canvas.getContext("2d");
const drawBarConfig = (config)=>{
    ...
    for(let i = 0; i < config.length; i++){
        ...
        if (config[i].inUse) context.fillStyle = "#FF0022";
        else if (config[i].value === i + 1) context.fillStyle = "#000000";
        else context.fillStyle = "#0081A7";
        context.fillRect(barPosition, // Start in the bottom of the canvas.
        window.innerHeight, barWidth, barHeight);
    }
};
...
},{}],"9T5jE":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const bubbleSort = (array)=>{
    ...
    return {
        ...
        iterate () {
            ...
            if (array[j].value > array[j + 1].value) (0, _lists.swap)(array[j], array[j + 1]);
            j++;
        }
    };
};
const selectionSort = (array)=>{
    ...
    return {
        ...
        iterate () {
            if (this.done) return;
            if (j >= array.length) {
                if (i !== minIndex) (0, _lists.swap)(array[i], array[minIndex]);
                i++;
                j = i;
                minIndex = i;
                if (i >= array.length - 1) {
            ...
            array[j].inUse = true;
            if (array[minIndex].value > array[j].value) minIndex = j;
            j++;
        }
    };
};
